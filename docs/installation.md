...
The following user guide provides instructions for installing and running TKLABEL Agent using Docker, which is the primary runtime environment for the framework. For developers and contributors, we also provide instructions for setting up the [full development environment](#in-depth-guide-for-full-binaries-installation).

### Need updates from v0.7? 👉[How to update TKLABEL Agent](#how-to-update-agent-zero)
...
- Docker Desktop provides the runtime environment for TKLABEL Agent, ensuring consistent behavior and security across platforms
...
1.5. Create a Docker Hub account when prompted and sign in. This is required to pull the TKLABEL Agent container image.
...
2.1. Pull the TKLABEL Agent Docker image:
...
- Choose or create a directory on your machine where you want to store TKLABEL Agent's data
- This can be any location you prefer (e.g., `C:/agent-zero-data` or `/home/user/agent-zero-data`)
- This directory will contain all your TKLABEL Agent files, like the legacy root folder structure:
  ...
  - `.env` - Your API keys
  - `settings.json` - Your TKLABEL Agent settings

> [!TIP]
> Choose a location that's easy to access and backup. All your TKLABEL Agent data 
...
- The Web UI will open. TKLABEL Agent is ready for configuration!
...
> [!NOTE]
> After starting the container, you'll find all TKLABEL Agent files in your chosen 
...
3. Configure TKLABEL Agent
- Refer to the following sections for a full guide on how to configure TKLABEL Agent.

## Settings Configuration
TKLABEL Agent provides a comprehensive settings interface to customize various aspects of its functionality. Access the settings by clicking the "Settings"button with a gear icon in the sidebar.
...
# Choosing Your LLMs
The Settings page is the control center for selecting the Large Language Models (LLMs) that power TKLABEL Agent.  You can choose different LLMs for different roles:
...
> mixed in the vector database. Note that this will DELETE ALL of TKLABEL Agent's memory.
...
#### Selecting your model within TKLABEL Agent
...
## Using TKLABEL Agent on your mobile device
TKLABEL Agent's Web UI is accessible from any device on your network through the Docker container:
...
> [!NOTE]
> If you're running TKLABEL Agent directly on your system (legacy approach) instead of 
> using Docker, you'll need to configure the host manually in `run_ui.py` to run on all interfaces using `host="0.0.0.0"`.

For developers or users who need to run TKLABEL Agent directly on their system,see the [In-Depth Guide for Full Binaries Installation](#in-depth-guide-for-full-binaries-installation).

# How to update TKLABEL Agent

1. **If you come from the previous version of TKLABEL Agent (pre-0.7.1):**
- Your data is safely stored across various directories and files inside the TKLABEL Agent folder.
- To update to the new Docker runtime version, you need to save the following files and directories:
  ...
  - `/work_dir` - Working directory
  - `.env` - Your API keys
  - `/tmp/settings.json` - Your TKLABEL Agent settings
...
- Reach for the folder where you saved your data and copy it to the new TKLABEL Agent folder set during the installation process.
- TKLABEL Agent will automatically detect your saved data and use it across memory, knowledge, instruments, prompts and settings.

> [!IMPORTANT]
> Make sure to use the same embedding model you were using before, otherwise 
> you will have to re-index all of TKLABEL Agent's memory, therefore deleting all 
> your custom knowledge and memory.
>
> If you have issues loading your settings, you can try to delete the `/tmp/settings.json` 
> file and let TKLABEL Agent generate a new one.
...
> your TKLABEL Agent data directory (the chosen directory on your machine).
...
3. **Full Binaries**
- Using Git/GitHub: Pull the latest version of the TKLABEL Agent repository. 
...
- TKLABEL Agent is a framework. It's made to be customized, edited, enhanced. Therefore you need to install the necessary components to run it when downloading its full binaries. This guide will help you to do so.
- The following step by step instructions can be followed along with a video for this tutorial on how to make TKLABEL Agent work with its full development environment.
...
2. You don't necessarily need API keys: TKLABEL Agent can run with local models. For this tutorial though, we will leave it to the default OpenAI API. A guide for downloading Ollama along with local models is available [here](#installing-and-using-ollama-local-models).
...
6. Running TKLABEL Agent without Docker makes the process more complicated and it's thought for developers and contributors.
...
## 2. Download TKLABEL Agent
- You can clone the TKLABEL Agent repository (https://github.com/frdel/agent-zero) from GitHub if you know how to use Git. In this tutorial I will just show how to download the files.

1. Go to the TKLABEL Agent releases [here](https://github.com/frdel/agent-zero/releases).
2. The latest release is on the top of the list, click the "Source Code (zip)" button under "Assets" to download it.

<img src="res/setup/image-14-u.png" alt="TKLABEL Agent download" width="500"/>
<br><br>
...
2. In the terminal, navigate to your TKLABEL Agent folder using **"cd"** command. Replace the path with your actual TKLABEL Agent folder path.
~~~
cd C:\Users\frdel\Desktop\agent-zero
~~~
You should see your folder has changed on the next terminal line.

<img src="res/setup/image-15.png" alt="TKLABEL Agent cd" height="100"/>
<img src="res/setup/image-16.png" alt="TKLABEL Agent cd" height="100"/>
<br><br>

3. Create Conda environment using command **"conda create"**. After **"-n"** is your environment name, you can choose your own, i will use **"a0"** - short for TKLABEL Agent. After **"python"** is the Python version that Conda will install for you into this environment, right now, 3.12 works fine. **-y** skips confirmations.
~~~
...
Simply put, Docker is a way of running virtual computers on your machine. These are lightweight, disposable and isolated from your operating system, so it is a way to sandbox TKLABEL Agent.
- TKLABEL Agent only connects to the Docker container when it needs to execute code and commands. The frameworks itself runs on your machine.
...
> Docker socket to be used (requires password)."  This allows TKLABEL Agent to 
...
- TKLABEL Agent needs a Docker image to be pulled from the Docker Hub to be run, even when using the full binaries.
You can refer to the [installation instructions above](#windows-macos-and-linux-setup-guide) to run the Docker container and then resume from the next step. There are two differences:
  ...
  - You need to map the `/a0` volume to the location of your local TKLABEL Agent folder.
- Run the Docker container following the instructions.

## 5. Run the local TKLABEL Agent instance
Run the TKLABEL Agent with Web UI:
~~~
python run_ui.py
~~~
...
- Open the URL shown in terminal in your web browser. You should see the TKLABEL Agent interface.

## 6. Configure TKLABEL Agent
Now we can configure TKLABEL Agent - select models, settings, API Keys etc. Refer to the [Usage](usage.md#agent-configuration) guide for a full guide on how to configure TKLABEL Agent.

## 7. Configure TKLABEL Agent RFC
TKLABEL Agent needs to be configured further to redirect some functions to the Docker container. This is crucial for development as A0 needs to run in a standardized environment to support all features.
...
9. Now you have the full development environment to work on TKLABEL Agent.
...
### Conclusion
After following the instructions for your specific operating system, you should have TKLABEL Agent successfully installed and running. You can now start exploring the framework's capabilities and experimenting with creating your own intelligent agents. 

If you encounter any issues during the installation process, please consult the [Troubleshooting section](troubleshooting.md) of this documentation or refer to the TKLABEL Agent [Skool](https://www.skool.com/agent-zero) or [Discord](https://discord.gg/Z2tun2N3) community for assistance.
