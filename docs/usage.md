# Usage Guide
This guide explores usage and configuration scenarios for TKLABEL Agent. You can consider this as a reference post-installation guide.

![Utility Message with Solutions](res/memory-man.png)

## Basic Operations
TKLABEL Agent provides several basic operations through its interface:

### Restart Framework
The Restart button allows you to quickly restart the TKLABEL Agent framework without using the terminal:
...
### Action Buttons
Located beneath the chat input box, TKLABEL Agent provides a set of action buttons for enhanced control and visibility:
...
### File Browser: Manage files in the TKLABEL Agent environment
...
### File Attachments
TKLABEL Agent supports direct file attachments in the chat interface for seamless file operations:
...
## Tool Usage
TKLABEL Agent's power comes from its ability to use [tools](architecture.md#tools). Here's how to leverage them effectively:

- **Understand Tools:** TKLABEL Agent includes default tools like knowledge (powered by SearXNG), webpage content, code execution, and communication. Understand the capabilities of these tools and how to invoke them.

## Example of Tools Usage: Web Search and Code Execution
Let's say you want TKLABEL Agent to perform some financial analysis tasks. Here's a possible prompt:
...
> [!NOTE]
> The first run of `code_execution_tool` may take a while as it downloads and builds the TKLABEL Agent Docker image. Subsequent runs will be faster.

This example demonstrates how to combine multiple tools to achieve an analysis task. By mastering prompt engineering and tool usage, you can unlock the full potential of TKLABEL Agent to solve complex problems.

## Multi-Agent Cooperation
One of TKLABEL Agent's unique features is multi-agent cooperation.
...
* **Hierarchy:** TKLABEL Agent uses a [hierarchical structure](architecture.md#agent-hierarchy-and-communication), with superior agents delegating tasks to subordinates.  This allows for structured problem-solving and efficient resource allocation.

![](res/physics.png)
![](res/physics-2.png)

## Prompt Engineering
Effective prompt engineering is crucial for getting the most out of TKLABEL Agent. Here are some tips and techniques:

* **Be Clear and Specific:** Clearly state your desired outcome.  The more specific you are, the better TKLABEL Agent can understand and fulfill your request.  Avoid vague or ambiguous language.
...
## Voice Interface
TKLABEL Agent provides both Text-to-Speech (TTS) and Speech-to-Text (STT) capabilities for natural voice interaction:
...
TKLABEL Agent supports KaTeX for beautiful mathematical typesetting:
...
### File Browser
TKLABEL Agent provides a powerful file browser interface for managing your workspace:
...
> [!NOTE]
> The files browser allows the user to go in the TKLABEL Agent root folder if you click the `Up` button, but the working directory of Agents will always be `/work_dir`
> 
- **File Operations**:
...
> [!TIP]
> The File Browser integrates seamlessly with TKLABEL Agent's capabilities. You can reference files directly in your conversations, and the agent can help you manage, modify, and organize your files.